...
public:
    ...
    {
        this->vertices.clear();
        for (std::vector<int> &ti : this->triangle_indices) ti.clear();

        this->vertices_size = 0;
        for (size_t &triangle_indices_size : this->triangle_indices_sizes) triangle_indices_size = 0;
    }
...
public:
    explicit TriangleSelectorMmuGui(const TriangleMesh& mesh, const std::vector<std::array<float, 4>> &colors, const std::array<float, 4> &default_volume_color)
        : TriangleSelectorGUI(mesh), m_colors(colors), m_default_volume_color(default_volume_color), m_gizmo_scene(colors.size() + 1) {
